...
export class Level {
    ...
    constructor(levelArgs: {seed: number, mobs: Point[], torches: Point[]}, inputs: Inputs, game: Game) {
      ...
      levelArgs.mobs.map(m => this.addMob(new Mob(m.x, m.y)))
      this.goal = {x: 23, y: 13};
      ...
      this.tickInterval = setInterval(() => this.tick(), 1000/UPS);
      this.renderInterval = setInterval(() => requestAnimationFrame(() => this.render()), 1000/FPS);
    }
  
    tick() {
      this.tickCount++;
      this.player.move(this.inputs, this.maze, this.torches);
